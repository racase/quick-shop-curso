# ai-product-generation Specification

## Purpose
Provides an AI-powered product field generation capability for admin users. Admins can submit a natural-language prompt describing a product, and the backend returns structured product fields (name, description, price, stock, image_url) generated by an LLM via OpenRouter. The OpenRouter API key is kept exclusively on the backend.
